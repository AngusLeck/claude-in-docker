# lib/installer.sh - Functions for install.sh
...
configure_credentials() {
    ...
    local method=$(gum choose "Enter credentials interactively" "Edit config file in editor")

    if [[ "$method" == "Enter credentials interactively" ]]; then
        local DEFAULT_GIT_NAME=$(git config --global user.name 2>/dev/null || echo "")
        local DEFAULT_GIT_EMAIL=$(git config --global user.email 2>/dev/null || echo "")
        local DEFAULT_GH_TOKEN=""
        ...
        DEFAULT_CLAUDE_CREDS=""
        if [[ "$(uname)" == "Darwin" ]]; then
            DEFAULT_CLAUDE_CREDS=$(security find-generic-password -s "Claude Code-credentials" -w 2>/dev/null || echo "")
        fi

        if [[ -n "$DEFAULT_CLAUDE_CREDS" ]]; then
            if gum confirm "Found Claude credentials in Keychain. Use them?"; then
                CLAUDE_CREDENTIALS="$DEFAULT_CLAUDE_CREDS"
            else
                echo "(Paste the JSON from ~/.claude/.credentials.json or leave empty)"
                CLAUDE_CREDENTIALS=$(gum input --password --char-limit 0 --placeholder "Claude credentials JSON (optional)")
            fi
        else
...
install_completions() {
    ...
    if [[ -n "$BASH_VERSION" ]] || [[ -f "$HOME/.bashrc" ]]; then
        mkdir -p "$bash_dir"
        cp "$REPO_DIR/completions/claude-docker.bash" "$bash_dir/claude-docker"
        echo "Bash completions installed."
        installed=true
    fi

    if [[ -n "$ZSH_VERSION" ]] || [[ -f "$HOME/.zshrc" ]]; then
        local zsh_dir="$HOME/.zfunc"
        mkdir -p "$zsh_dir"
        cp "$REPO_DIR/completions/_claude-docker" "$zsh_dir/_claude-docker"
