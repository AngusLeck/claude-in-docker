...
configure_credentials() {
    ...
    if [[ "$method" == "Enter credentials interactively" ]]; then
        ...
        echo
        echo "NPM Token (for private registry access):"
        local DEFAULT_NPM_TOKEN="${NPM_TOKEN:-}"
        if [[ -n "$DEFAULT_NPM_TOKEN" ]]; then
            if gum confirm "Found NPM_TOKEN in your environment. Use it?"; then
                NPM_TOKEN="$DEFAULT_NPM_TOKEN"
            else
                NPM_TOKEN=$(gum input --password --placeholder "NPM token (leave empty to skip)")
            fi
        else
            echo "Tip: Get a token with 'npm token create --read-only' or set NPM_TOKEN in your shell"
            NPM_TOKEN=$(gum input --password --placeholder "NPM token (leave empty to skip)")
        fi

        CREDENTIALS_CONTENT="# Claude-in-Docker credentials
# Generated by install.sh on $(date)
# Note: Claude credentials are detected automatically at runtime
